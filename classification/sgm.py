...
def load_images(left_name, right_name, parameters):
    ...
    left = cv2.resize(left,(left.shape[1]//5, left.shape[0]//5))
    left = cv2.GaussianBlur(left, parameters.bsize, 0, 0)
    right = cv2.imread(right_name, 0)
    right = cv2.resize(right, (right.shape[1] // 5, right.shape[0] // 5))
...
def sgm():
    ...
    parser.add_argument('--left', default='../dataset/JPEGImages/DSC_2410.JPG', help='name (path) to the left image')
    parser.add_argument('--right', default='../dataset/JPEGImages/DSC_2411.JPG', help='name (path) to the right image')
    ...
    parser.add_argument('--disp', default=100, type=int, help='maximum disparity for the stereo pair')
