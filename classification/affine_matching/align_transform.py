...
class Align():
    ...
    def warp_points(self, pts, M):
        '''
        pts assume [x
                    y]
        '''
        # convert the pts array to a perpendicular coordinates
        cols=pts.shape[1]
        one_array=np.ones(cols)
        pts=np.vstack([pts,one_array.T])
        target_point=M@pts
        return target_point
